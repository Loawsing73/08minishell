...
void print_pipeline(t_ast_node *node, int depth)
{
	...
	printf("├── left_cmd:\n");
	print_ast(node->data.pipeline.left_cmd, depth + 2);
	print_indent(depth + 1);
	printf("└── right_pipeline:\n");
	print_ast(node->data.pipeline.right_pipeline, depth + 2);
}
...
void print_command(t_ast_node *node, int depth)
{
	...
	print_indent(depth + 1);
	printf("├── cmd_word:\n");
	print_ast(node->data.command.cmd_word, depth + 2);
	
	if (node->data.command.arguments)
	{
		print_indent(depth + 1);
		printf("├── arguments:\n");
		print_ast(node->data.command.arguments, depth + 2);
	}
	...
	if (node->data.command.redirections)
	{
		print_indent(depth + 1);
		printf("└── redirections:\n");
		print_ast(node->data.command.redirections, depth + 2);
	}
}
...
{
	printf("WORD: ");
	if (node->data.word.value)
		printf("%s", node->data.word.value);
	else
		printf("NULL");
	
	if (node->data.word.needs_expansion)
		printf(" [NEEDS_EXPANSION]");
...
{
	printf("REDIRECTION: ");
	if (node->data.redirection.redir_type == LESS)
		printf("<");
	else if (node->data.redirection.redir_type == GREAT)
		printf(">");
	else if (node->data.redirection.redir_type == D_LESS)
		printf("<<");
	else if (node->data.redirection.redir_type == D_GREAT)
		printf(">>");
	printf("\n");
	
	print_indent(depth + 1);
	printf("└── target:\n");
	print_ast(node->data.redirection.target, depth + 2);
}

void print_quoted_string(t_ast_node *node, int depth)
{
	printf("QUOTED_STRING: ");
	if (node->data.quoted_string.quote_type == QUOTE)
		printf("'");
	else
		printf("\"");
	printf("\n");
	
	if (node->data.quoted_string.needs_expansion)
	{
		print_indent(depth + 1);
		printf("[NEEDS_EXPANSION]\n");
	}
	
	if (node->data.quoted_string.content_list)
	{
		print_indent(depth + 1);
		printf("└── content:\n");
		print_ast(node->data.quoted_string.content_list, depth + 2);
	}
}
...
void print_ast(t_ast_node *node, int depth)
{
	...
	if (node->type == NODE_PIPELINE)
		...
		print_word(node, depth);
	else if (node->type == NODE_REDIRECTION)
		print_redirection(node, depth);
	else if (node->type == NODE_QUOTED_STRING)
		print_quoted_string(node, depth);
	else if (node->type == NODE_VARIABLE)
